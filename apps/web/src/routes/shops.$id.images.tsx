...
function ShopImagesPage() {
  ...
  const { data, isLoading, refetch } = useGetShopsByIdImages(id);
  ...
  return (
    <div className="container mx-auto max-w-4xl space-y-8 px-6 py-8 lg:px-12">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold">Shop Images</h1>
        <div>
          <input
            accept="image/*"
            className="hidden"
          ...
          {images.map((img) => {
            ...
            return (
              <div className="group relative overflow-hidden rounded-lg" key={img.id}>
                <img alt="Shop" className="aspect-video w-full object-cover" src={src} />
                <button
