...
import type { WineWithWinemaker, WineData, WineFilters } from './wines.repository'
...
export const winesService = {
  ...
  async replaceWine(id: string, userId: string, userRole: string, data: WineData): Promise<WineWithWinemaker> {
    const wine = await winesRepository.findById(id)
    if (!wine) throw new Error('NOT_FOUND')
