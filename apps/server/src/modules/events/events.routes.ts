...
function handleError(e: unknown) {
  if (e instanceof Error) {
    ...
    if (e.message === 'INVALID_DATES') return status(422, 'Invalid dates: start must be in future, end must be after start')
  }
  throw e
}
...
export const eventsRoutes = new Elysia()
  ...
  .get(
    ...
    async ({ params, query }) => {
      try {
        return await eventsService.listComments(params.id, { page: query.page, limit: query.limit })
      } catch (e) {
        return handleError(e)
      }
    },
