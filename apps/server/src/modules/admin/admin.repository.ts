  ...
  async listAllReviews(pagination: {
    ...
    const [productRows, winemakerRows, productCount, winemakerCount] = await Promise.all([
      db.query.productReviews.findMany({
        ...
        columns: { id: true, userId: true, productId: true, rating: true, body: true, createdAt: true },
      }),
      db.query.winemakerReviews.findMany({
        where: isNull(winemakerReviews.deletedAt),
        columns: { id: true, userId: true, winemakerId: true, rating: true, body: true, createdAt: true },
      }),
      db.select({ value: count() }).from(productReviews).where(isNull(productReviews.deletedAt)),
      db.select({ value: count() }).from(winemakerReviews).where(isNull(winemakerReviews.deletedAt)),
    ])
    ...
    const total =
      Number(productCount[0]?.value ?? 0) + Number(winemakerCount[0]?.value ?? 0)
